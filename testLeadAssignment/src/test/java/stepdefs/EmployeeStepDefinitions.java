...
public class EmployeeStepDefinitions {
	...
	ExtentReports extent = new ExtentReports(
			path + "./report/result" + d.toString().replace(":", "_").replace(" ", "_") + ".html", true);
...
//For creating each test
	...
	public void writeDataInSheet(Object[] post) {
		...
		try {
			...
			FileOutputStream outputStream = new FileOutputStream(
					path + "//report//testResult" + d.toString().replace(":", "_").replace(" ", "_") + ".xlsx");
	...
	public void accessEmployee() {
		try {
			...
			response = request.log().all()
					.header("Cookie", "ezCMPCCS=true;",
							" ezepvv=0;active_template::133674=pub_site.1594754196; ezovuuidtime_133674=1594754197;"
									+ cValue)
					.header("Content-Type", "application/json").pathParam("id", employeeId).get("/employee/{id}");
		...
		try {
			...
			response = request.log().all()
					.header("Cookie", "ezCMPCCS=true;",
							" ezepvv=0;active_template::133674=pub_site.1594754196; ezovuuidtime_133674=1594754197;"
									+ cValue)
					.header("Content-Type", "application/json").pathParam("id", employeeId)
					.body("{\"name\":\"Archana\",\"salary\":\"1234567\",\"age\":\"23\"}").put("/update/{id}");
	...
	public void deleteEmployee() {
		try {
			...
			response = request.log().all()
					.header("Cookie", "ezCMPCCS=true;",
							" ezepvv=0;active_template::133674=pub_site.1594754196; ezovuuidtime_133674=1594754197;"
									+ cValue)
					.header("Content-Type", "application/json").pathParam("id", employeeId).delete("/delete/{id}");
		...
		String[] StringData = { "Created Employee", reaponseJsonString, "Employee Details", jsonData,
				"Updated Employee", jsonDataPut, "Deleted Employee", jsonDataDelete };
